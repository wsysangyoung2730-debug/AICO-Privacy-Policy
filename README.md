...
`docs/privacy-policy.md`만 원본으로 관리합니다. 문서 상단의 `시행일`, `최종 수정일`, `버전`을 변경하고, 하단 변경 이력의 첫 행에도 같은 버전과 시행일을 적습니다.

Vercel은 배포할 때 `build_privacy_page.rb`를 실행하여 `docs/index.html`을 다시 생성합니다. Vercel 프로젝트와 이 저장소의 `main` 브랜치를 연결하면, 이후 푸시할 때마다 상단 문서 정보와 본문·변경 이력이 같은 원본을 기준으로 자동 배포됩니다. 값이 서로 다르면 빌드가 실패하여 불일치한 문서가 공개되는 것을 막습니다.
